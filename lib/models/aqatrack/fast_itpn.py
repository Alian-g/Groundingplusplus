...
class Attention(nn.Module):
    def __init__(
            ...
            deepnorm=False, subln=False, use_gate=True
    ):
        super().__init__()
        ...
        # ===== GateAttention（仅使用 gheat）：sigmoid(Wg * gheat) =====
        self.use_gate = use_gate
        if self.use_gate:
            # 将每个位置的标量 gheat → 每个 head 的 gate logit
            self.gheat_proj = nn.Linear(1, num_heads, bias=True)
            nn.init.zeros_(self.gheat_proj.bias)  # 可选：偏置置零

    def forward(self, x, rel_pos_bias=None, attn_mask=None, require_attn=False, gheat=None):
        B, N, C = x.shape
        ...
        q = q * self.scale
        attn = (q.float() @ k.float().transpose(-2, -1)) if self.qk_float else (q @ k.transpose(-2, -1))
        ...
        if attn_mask is not None:
            attn = attn.masked_fill(~attn_mask[:, None, None, :].bool(), float("-inf"))
        ...
        # ---- GateAttention：只作用在 search 256，上 gate = sigmoid(Wg * gheat) ----
        # if self.use_gate and gheat is not None:
        #     n_cls, n_temp1, n_temp2, n_search = 1, 64, 64, 256
        #     y_cls    = y[:, :, :n_cls, :]
        #     y_temp1  = y[:, :, n_cls:n_cls+n_temp1, :]
        #     y_temp2  = y[:, :, n_cls+n_temp1:n_cls+n_temp1+n_temp2, :]
        #     y_search = y[:, :, -n_search:, :]  # [B, heads, 256, d]

        #     # 规范化并展平 gheat → [B,256]
        #     if gheat.dim() == 4:      # [B,1,16,16]
        #         g_vec = gheat.flatten(2).squeeze(1)       # [B,256]
        #     elif gheat.dim() == 3:    # [B,16,16]
        #         g_vec = gheat.flatten(1)                  # [B,256]
        #     else:                      # [B,256]
        #         g_vec = gheat

        #     # per-image min-max 到 [0,1]
        #     gmin = g_vec.min(dim=1, keepdim=True)[0]
        #     gmax = g_vec.max(dim=1, keepdim=True)[0]
        #     g_vec = (g_vec - gmin) / (gmax - gmin + 1e-6)  # [B,256]

        #     # 由 gheat 直接产生每 token、每 head 的 gate logits
        #     # [B,256,1] --Linear(1→heads)--> [B,256,heads]
        #     gate_logits = self.gheat_proj(g_vec.unsqueeze(-1))
        #     gate = torch.sigmoid(gate_logits).permute(0, 2, 1).unsqueeze(-1)  # [B, heads, 256, 1]

        #     # 乘性门控（与论文最优设置一致）
        #     y_search = y_search * gate

        #     # 拼接回来
        #     y = torch.cat([y_cls, y_temp1, y_temp2, y_search], dim=2)
            ...
            if self.postnorm:
                ...
                if self.attn is not None:
                    if require_attn:
                        ...
                        x = x + self.drop_path(
                            self.gamma_1 *x_m )
                    else:#走这里，在这里进行attention
                        x = x + self.drop_path(
                            self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, attn_mask=attn_mask,gheat=gheat))
...
class Fast_iTPN(BaseBackbone):
    ...
    def forward_features_pe(self, z,x,soft_token_template_mask):
        B, C, H, W = x.shape
        z, z1 = z[0], z[1]
        z = self.patch_embed(z)#20，192，32，32
        z1 = self.patch_embed(z1)#20，192，32，32
        x = self.patch_embed(x)#20，192，64，64

        for blk in self.blocks[:-self.num_main_blocks]:
            x = blk(x)#20，768，16，16
            z = blk(z)#20，768，8，8
            z1 = blk(z1)#20，768，8，8

        x = x.flatten(2).transpose(1, 2)#20，256，768
        z = z.flatten(2).transpose(1, 2)#20，64，768
        z1 = z1.flatten(2).transpose(1, 2)#20，64，768

        token_type_search = self.token_type_search.expand(B, x.shape[1], -1)#20，256，768
        token_type_template_bg = self.token_type_template_bg.expand(B, z.shape[1], -1)#20，64，768
        token_type_template_fg = self.token_type_template_fg.expand(B, z.shape[1], -1)#20，64，768
    ...
    def forward_features_stage3(self,x,update_cls_token,pos_embed=None,gbox=None,gbox_valid=None):
        B, L, D = x.shape  # 这里 L 一般是 template_tokens + search_tokens（还没拼 cls）
        ...
        if has_cls:
            ...
            x = torch.cat([cls_tokens, x], dim=1)  # (B, 1+L, D)
        ...
        # ===== 新增：根据 gbox 给 search token 加 inside/outside 可学习 embedding =====
        if gbox is not None:
            z_len = self.pos_embed_z.shape[1]
            x_len = self.pos_embed_x.shape[1]

            cls_offset = 1 if self.cls_token is not None else 0
            search_start = cls_offset + z_len
            search_end = search_start + x_len

            search_inside_mask = self._build_search_token_mask_from_gbox(
                gbox=gbox,
                x_len=x_len,
                device=x.device,
                box_format="cxcywh"
            ...
            search_inside_mask = search_inside_mask.unsqueeze(-1)  # (B, x_len, 1)

            in_emb = self.gbox_in_embed.expand(B, x_len, D)
            out_emb = self.gbox_out_embed.expand(B, x_len, D)

            search_region_emb = torch.where(
                search_inside_mask,
                in_emb,
                out_emb
            )  # (B, x_len, D)

            # 低置信度 gbox 不注入先验，避免错误框污染 tracker
            if gbox_valid is not None:
                gbox_valid = gbox_valid.to(x.device).view(B, 1, 1)
                search_region_emb = search_region_emb * gbox_valid.to(search_region_emb.dtype)

            x[:, search_start:search_end, :] = (
                x[:, search_start:search_end, :] + search_region_emb
            )
        # ===== 新增结束 =====
